...
- 设置：应用外观（跟随系统 / 明亮 / 暗黑）、自动拉起守护进程开关、内存预算、运行环境、网络代理和模型下载源
  ...
  - 运行环境：一键安装 llama.cpp（脚本克隆固定 revision 并编译 llama-server）与剩余 legacy Python worker（mlx-lm、sherpa-onnx、Qwen3-TTS）所需的环境（仓库 `.build/` 下，与 daemon 探测路径一致），支持取消与失败重试；Kokoro 与 Qwen3-ASR 已迁移到 Runner（uv 受管环境，见「Runner 引擎」区块）
