...
- [使用者与开发者统一文档体系](2026-09-14-unified-guide-site.md)——`implemented`
  （docs/guide/ 单页静态综合文档站点：将用户指南与开发者参考深度整合，建立 6 大主题模块与 27 个分级锚点，
  涵盖 DMG/源码构建、7 大 Runner 引擎实操与环境准备、模型生命周期、Chat/STT/TTS 实操、CLI/API 矩阵、
  系统架构调度机制与 10 大按症状故障排查；零外部依赖可离线打开，与项目主页、README 互链。）
...
- [TTS 音色滚轮快速切换与即时试听](2026-09-13-tts-voice-wheel-selection.md)——`implemented`
  （共享 `VoiceSelectionControl`：NSPopUpButton 子类滚轮步进——触控板手势按阈值累积、滚轮类设备一次物理滚动固定一行，settle 后防抖持久化 + 可关自动试听，AVAudioPlayer 可打断播放；运行状态页、模型详情页、管理页三处平行试听实现收敛为一，daemon 与 wire 契约零改动。）
...
- [Runner chat 流式 delta 无丢失缓冲](2026-09-12-runner-chat-stream-lossless-buffering.md)——`implemented`
  （RunnerProvider 流式转发改用无界缓冲消除慢客户端下的静默丢字；协议无流控是理由，引入流控时回改。）
