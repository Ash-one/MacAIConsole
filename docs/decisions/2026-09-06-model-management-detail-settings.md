...
4. 无法在统一的界面中对模型进行详细设置（如调整上下文长度、修改模型注册 ID、在访达中定位文件、触发重载等）。
...
1. **统一覆盖三大列表**：
   - 已注册模型（`ModelEntry`）：展示注册 ID、Runner 标识、进程隔离级别、daemon 裁决原因（`provider_selection_reason`）、请求来源（`requested_provider`）、来源路径、实时运行指标（生效设备、内存占用等），并支持在弹窗内直接更改 ID、调节 LLM 上下文长度、启动/停止和删除注销。
