...
1. **绝对路径硬编码**：`apps/MacAIConsole/Sources/MacAIConsole/DaemonController.swift` 在探测 `aiworkd` 二进制时，硬编码了开发者个人的本地路径（`Projects/MacAI`），导致外部克隆或构建缺少 `AIWORKD_PATH` 时无法启动 daemon；
