...
# 兼容别名：tests 早期使用 decode_frame 表示「解码已含头部的完整 frame」。
def decode_frame(wire: bytes) -> dict:
    """解码一个完整的 wire frame（4 字节头 + payload）。"""
    if len(wire) < HEADER_SIZE:
        raise ValueError("frame shorter than header")
    (length,) = struct.unpack(">I", wire[:HEADER_SIZE])
    return json.loads(wire[HEADER_SIZE : HEADER_SIZE + length].decode("utf-8"))
