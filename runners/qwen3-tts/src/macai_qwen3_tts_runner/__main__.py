...
用法：
    python -m macai_qwen3_tts_runner            # 常驻协议服务
    python -m macai_qwen3_tts_runner --probe    # 只读环境探针（manifest probe 用）
"""
...
if __package__ in (None, ""):  # pragma: no cover - direct script execution
    ...
    from macai_qwen3_tts_runner import protocol
else:
    from macai_qwen3_tts_runner import protocol

from macai_qwen3_tts_runner.engine import Qwen3TtsEngine

PROTOCOL_VERSION = "macai.runner.v1"
RUNNER_ID = "org.macai.qwen3-tts"
...
def run_probe() -> int:
    ...
    print("qwen3-tts runner probe ok")
    return 0
    ...
    while True:
        ...
        if frame_type == "initialize":
            ...
            try:
                ...
                engine = Qwen3TtsEngine(model_root)
                _send(
                    {
                        "protocol": PROTOCOL_VERSION,
                        "type": "loaded",
