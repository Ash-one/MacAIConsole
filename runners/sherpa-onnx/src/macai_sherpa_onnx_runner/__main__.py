...
用法：
    python -m macai_sherpa_onnx_runner            # 常驻协议服务
    python -m macai_sherpa_onnx_runner --probe    # 只读环境探针（manifest probe 用）
"""
...
if __package__ in (None, ""):  # pragma: no cover - direct script execution
    ...
    from macai_sherpa_onnx_runner import protocol
else:
    from macai_sherpa_onnx_runner import protocol

from macai_sherpa_onnx_runner.engine import AudioInputError, SherpaOnnxEngine

PROTOCOL_VERSION = "macai.runner.v1"
RUNNER_ID = "org.macai.sherpa-onnx"
...
def run_probe() -> int:
    ...
    print("sherpa-onnx runner probe ok")
    return 0
...
def main() -> int:
    ...
    engine: SherpaOnnxEngine | None = None
    _send(
        {
    ...
    while True:
        ...
        if frame_type == "initialize":
            ...
            try:
                ...
                engine = SherpaOnnxEngine(model_root)
                _send(
                    {
                        "protocol": PROTOCOL_VERSION,
                        "type": "loaded",
            ...
            if not audio:
                ...
                continue
            if language not in (None, "Chinese", "zh"):
                _send(
                    {
                        "protocol": PROTOCOL_VERSION,
                        "type": "error",
                        ...
                        "payload": {
                            ...
                            "message": "sherpa-onnx zh-int8-2025 accepts only Chinese",
                            "retryable": False,
                        },
                    }
                )
                ...
                _send(
                    {
                        ...
                        "type": "result",
                        "id": frame_id,
                        "payload": {"text": text, "language": "Chinese", "device": "cpu"},
                    }
                )
